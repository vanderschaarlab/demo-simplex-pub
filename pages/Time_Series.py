...
def load_trained_model(model, trained_model_state_path):
    model.load_state_dict(
        torch.load(trained_model_state_path, map_location=torch.device("cpu"))
    )
    model.eval()
    return model
...
def get_simplex_residual(test_latent):
    return torch.sqrt(torch.sum((test_latent - simplex.latent_approx()) ** 2)).item()


def get_all_bar_one_simplex_residuals(test_latents, test_patient_id):
    residuals = []
    for idx, test_latent in enumerate(test_latents):
        if idx != test_patient_id:
            residuals.append(
                torch.sqrt(
...
def get_residual_percentile_rank(test_latents, test_patient_id):

    return stats.percentileofscore(
        get_all_bar_one_simplex_residuals(test_latents, test_patient_id),
        get_simplex_residual(test_latents[test_patient_id]),
    )
...
def get_ccs(
    ...
    corpus_correctness_score = 0
    for corpus_id, corpus_member_data in enumerate(result):
        corpus_correctness_score += (
            100
            * corpus_member_data[0]
            * int(
                apply_sort_order(corpus_predictions, sort_order)[corpus_id].item()
                == apply_sort_order(corpus_targets, sort_order)[corpus_id]
            )
            * int(
                apply_sort_order(corpus_predictions, sort_order)[corpus_id].item()
                == test_predictions[test_patient_id].item()
            )
        )
    return corpus_correctness_score
...
# Load fitted SimplEx
with open(
    f"./resources/trained_models/{model_name2trained_model_repo_name['Recurrent Neural Net']}/Time series Prostate Cancer/simplex_reg=1.0_reg2={reg_factor_2}.pkl",
    "rb",
) as f:
    simplex = pkl.load(f)
...
def user_input_features():
    ...
    example_importance_threshold = st.sidebar.slider(
        "Minimum Example Importance", 0.0, 1.0, 0.05
    )
...
if test_patient_last_time_step_idx >= 3:
...
elif test_patient_last_time_step_idx == 2:
    test_patient_df = pd.DataFrame(
        simplex.test_examples[test_patient_id][
            0:test_patient_last_time_step_idx, :
        ].numpy(),
        columns=feature_names,
        index=["(t_max) - 1", "(t_max)"],
    )
elif test_patient_last_time_step_idx == 1:
    test_patient_df = pd.DataFrame(
        simplex.test_examples[test_patient_id][
            0:test_patient_last_time_step_idx, :
        ].numpy(),
...
for col_name, rescale_value in rescale_dict.items():
    test_patient_df[col_name] = test_patient_df[col_name].apply(
...
corpus_correctness_score = get_ccs(
    result,
    sort_order,
    test_patient_id,
    corpus_predictions,
...
explanation_quality_score = (
    0
    if get_simplex_residual(test_latents[test_patient_id]) == 0
    else (100 - get_residual_percentile_rank(test_latents, test_patient_id)) * 0.8 + 10
)
...
# col1, *other_cols, penultimate_col, col_last = st.columns(4)
# with col1:
#     st.markdown(hide_table_row_index, unsafe_allow_html=True)
#     st.table(
#         pd.DataFrame(
#             data={
#                 "Explanation Quality": [f"{explanation_quality_score:.4g}%"],
#                 "Corpus Correctness Score": [f"{corpus_correctness_score:.1f}%"],
#             }
#         )
#     )
