...
def get_ccs(
    ...
    for corpus_id, corpus_member_data in enumerate(result):
        corpus_correctness_score += (
            ...
            * int(
                apply_sort_order(corpus_predictions, sort_order)[corpus_id]
                == apply_sort_order(corpus_targets, sort_order)[corpus_id]
            )
            * int(
                apply_sort_order(corpus_predictions, sort_order)[corpus_id]
                == test_predictions[test_patient_id]
            )
        )
    return corpus_correctness_score


dropdown_col1, dropdown_col2, *other_cols, col5, col6 = st.columns(6)
with dropdown_col1:
...
# Load fitted SimplEx
with open(
    f"./resources/trained_models/{model_name2trained_model_repo_name[model_name]}/{data_source}/simplex_reg=1.0_reg2={reg_factor_2}.pkl",
    "rb",
) as f:
    simplex = pkl.load(f)
